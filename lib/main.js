...
const keywordRegExp = /\\usepackage\s*(?:\[.*\])?\s*{([^}]+)}/g
  ...
  provideIntentionsHighlight () {
    return {
      ...
      getIntentions: ({textEditor, visibleRange}) => {
        ...
        textEditor.scanInBufferRange(keywordRegExp, visibleRange, ({range, match}) => {
          console.log(match)
          if (match[1]) {
            matches.push(global.doc.search(match[1]).then(results => {
              if (results.length > 0) {
                return {
                  range: range,
